...
import { Category, UpdateCategoryDto } from '../apis/categoryApi';

interface CategoryEditModalProps {
  open: boolean;
  onClose: () => void;
  onUpdate: (categoryData: UpdateCategoryDto) => Promise<void>;
  category: Category;
}
...
const CategoryEditModal: React.FC<CategoryEditModalProps> = ({
  ...
  // Category prop'u değiştiğinde form data'sını güncelle
  useEffect(() => {
    if (category) {
      setFormData({
        name: category.name || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error in CategoryEditModal:', error);
    } finally {
      setLoading(false);
    }
  };
